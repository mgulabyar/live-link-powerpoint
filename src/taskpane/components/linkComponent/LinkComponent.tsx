...
const LinkComponent: React.FC<{ onLinkSuccess: () => void }> = ({ onLinkSuccess }) => {
  ...
  const fetchExcelDocuments = async () => {
    ...
    try {
      ...
      console.error("Fetch Files Error:", e);
      setStatusMessage({
        text: `API Connection Failed: ${e.message || "Failed to reach backend"}.`,
        severity: "error",
      });
    } finally {
      setFetchingFiles(false);
  ...
  const handleInsertLinkToSlide = async () => {
    ...
    try {
      ...
      await PowerPoint.run(async (context: any) => {
        ...
        // UPDATED: Overlapping/Stacking Bug hal karne ka logic
        if (selectedShapes.items.length > 0) {
          const activeShape = selectedShapes.items[0];
          activeShape.load(["left", "top", "tags"]);
          await context.sync();
          ...
          // Agar user ne PowerPoint par koi shape select nahi ki (Excel se bhejte waqt)
          // To slides par pehle se maujuda shapes ki ginti kar ke ek cascading offset calculate karenge
          const offsetCount = shapes.items.length;
          insertOptions.imageLeft = 50 + (offsetCount * 25);
          insertOptions.imageTop = 50 + (offsetCount * 25);
        }
