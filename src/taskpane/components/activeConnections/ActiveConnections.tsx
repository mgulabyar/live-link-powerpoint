...
//////////////////////////////// successfuly working me ha only pwerpoint.context.slides method use krna ha 
...
const ActiveConnections: React.FC<ActiveConnectionsProps> = ({ pptLinks, onLinkSuccess }) => {
  ...
  const handleRefreshLink = async (item: PPTLinkedItem) => {
    ...
    try {
      // 1. Target slide par switch karna (Slide focus switcher) [1]
      if (item.slideId) {
      ...
      await PowerPoint.run(async (context: any) => {
        // FIXED: Loads presentation.slides instead of getSelectedSlides to scan all slides [1]
        ...
        if (targetSlide) {
          ...
          if (targetShape) {
            ...
            // PowerPoint focus correction before image draw
            if (item.slideId) {
              await new Promise<void>((resolve) => {
                Office.context.document.goToByIdAsync(item.slideId, Office.GoToType.Slide, () => {
                  resolve();
        ...
        for (const item of safePptLinks) {
          ...
          if (targetSlide) {
            ...
            if (targetShape) {
              ...
              // Programmatic focus before bulk image draw
              if (item.slideId) {
                await new Promise<void>((resolve) => {
                  Office.context.document.goToByIdAsync(item.slideId, Office.GoToType.Slide, () => {
                    resolve();
