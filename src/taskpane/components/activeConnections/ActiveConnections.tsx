...
//         const activeDbIds = new Set<string>(response.data.map((dbLink: any) => dbLink.linkId));
//         setReachableIds(activeDbIds);
//       }
//     } catch (e) {
...
//       const dbLinksMap = new Map<string, any>();
//       response.data.forEach((dbLink: any) => {
//         dbLinksMap.set(dbLink.linkId, dbLink);
//       });
...
// import React, { useState, useEffect } from "react";
// import { Box, Typography, Button, Paper, IconButton, List, CircularProgress, Alert, Snackbar } from "@mui/material"; // Snackbar imported
// import { Refresh, Layers, Description, DeleteOutline } from "@mui/icons-material";
// import { getLinkDetails, getBulkLinkDetails, deleteLinkData } from "../services/api";
// import { deletePPTShape, PPTLinkedItem } from "../utils/officeHelpers";
...
const ActiveConnections: React.FC<ActiveConnectionsProps> = ({ pptLinks, onLinkSuccess }) => {
  ...
  // Stores the complete database objects map to fetch custom component names in real-time [1]
  ...
  const checkLinksReachability = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setDbLinksMap(linksMap); // Update database map in state [1]
      }
    } catch (e) {
      console.error("Failed to check reachability:", e);
    }
