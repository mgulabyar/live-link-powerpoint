...
export interface PPTLinkedItem {
  id: string;
  shapeId: string;
  slideId: string; // UPDATED: Slide ID track karne ke liye property add ki hai
  excelFileId: string;
  excelFileName: string;
  sheetName: string;
  rangeAddress: string;
...
export const getPPTLinkedItems = async (): Promise<PPTLinkedItem[]> => {
  return await PowerPoint.run(async (context: any) => {
    ...
    for (const slide of slides.items) {
      slide.load("id"); // UPDATED: Slide ID ko explicitly load kiya hai
      await context.sync();
      ...
      for (const shape of shapes.items) {
        ...
        if (isLinked && linkId) {
          linkedItems.push({
            id: linkId,
            shapeId: shape.id,
            slideId: slide.id, // UPDATED: Slide ID push ki ja rahi hai
            excelFileId,
            excelFileName,
            sheetName,
            rangeAddress,
